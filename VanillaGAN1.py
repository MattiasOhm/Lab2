...
# It is a simple GAN with the loss function asked for in task 2.1
...
base_dir = "runs/VanillaGANv1"
next_number = get_next_run_number(base_dir)
writer_fake = SummaryWriter(f'runs/VanillaGANv1/fake{next_number}')
writer_real = SummaryWriter(f'runs/VanillaGANv1/real{next_number}')
writer_loss = SummaryWriter(f'runs/VanillaGANv1/loss{next_number}')
...
for epoch in range(epochs):
    for batch, (real_images, _) in enumerate(loader): # We don't actually use the real labels, we just give a real image label =1, and fake image label =0
        ...
        # Evaluation
        if batch == 0:
            print(f'Epoch: {epoch}  | Discriminator loss: {G_loss:.3f} | Generator loss: {D_loss:.3f} | Iterations: {iterations}')
